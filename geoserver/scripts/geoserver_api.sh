...
function get_object() {
  ...
  case "${object_type}" in
    ...
    datastores)
      if [ -z "${workspace}" ] ; then
        clean_exit 30 "Workspace not provided."
      fi
    ...
    featuretypes)
      if [ -z "${workspace}" ] ; then
        clean_exit 31 "Workspace not provided."
      fi
      if [ -z "${datastore}" ] ; then
        clean_exit 31 "Datastore not provided."
      fi
      if [ -z "${object_name}" ] ; then
...
function update_object() {
  ...
  case "${object_type}" in
    ...
    services)
      if [ -z "${object_name}" ] ; then
        usage
        clean_exit 20 "No service provided."
      fi      
      data_file="${data_dir}/global/services/${object_name}.json"
      if ! [ -f ${data_file} ] ; then
        clean_exit 23 "Service json file not found: ${data_file}"
      fi
      uri="/services/${object_name}/settings"      
      ;;
    settings)
